            ...
            <tbody>
              ...
              {shown.map((r) => (
                <tr key={r.id}>
                  ...
                  <td style={{ fontWeight: 800 }}>${formatMoney(r.montoSinIva ?? r.subtotal ?? r.tax?.subtotal ?? 0)}</td>
                  <td style={{ fontWeight: 700 }}>${formatMoney(r.montoIva ?? r.iva ?? r.tax?.iva ?? 0)}</td>
                  <td style={{ fontWeight: 700 }}>${formatMoney(r.totalFactura ?? r.tax?.totalFactura ?? 0)}</td>
                  {isAdmin && (
                    <td>
                      <button
                        className="secondary"
